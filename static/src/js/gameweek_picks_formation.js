...
export class GameweekPicksFormation extends Component {
  ...
  async loadInitialData() {
    try {
      ...
      this.notification.add(_t("Failed to load initial data"), {
        type: "danger",
      });
    }
  }
  ...
  async syncGameweekPicks() {
    if (!this.state.gameweek_id || !this.state.manager_id) {
      this.notification.add(_t("Please select both gameweek and manager"), {
        type: "warning",
      });
      return;
    }
    ...
    try {
      ...
      const gameweek = this.state.gameweeks.find(
        (gw) => gw.id == this.state.gameweek_id
      );
      const manager = this.state.managers.find(
        (m) => m.id == this.state.manager_id
      );

      if (!gameweek || !manager) {
        this.notification.add(_t("Invalid gameweek or manager selection"), {
          type: "danger",
        });
        return;
      }
      ...
      await this.orm.call("fpl.gameweek.pick.lines", "sync_gameweek_picks", [
        manager,
        gameweek,
      ]);
      ...
      this.notification.add(_t("Gameweek picks synced successfully"), {
        type: "success",
      });
    } catch (error) {
      console.error("Error syncing gameweek picks:", error);
      this.notification.add(_t("Failed to sync gameweek picks"), {
        type: "danger",
      });
    }
  }
    ...
    try {
      ...
      if (gameweekPicks.length === 0) {
        this.notification.add(
          _t("No picks found for selected gameweek and manager"),
          { type: "info" }
        );
        return;
      }
      ...
      pickLines.forEach((line) => {
        this.state.playersData[line.id] = {
          ...
          team_code: parseInt(line.team_code),
        };
      });
      ...
      this.notification.add(_t("Failed to load formation data"), {
        type: "danger",
      });
    }
  }

  async openPlayerStats(ev) {
    ev.preventDefault();
    ev.stopPropagation();

    const playerId = parseInt(ev.currentTarget.dataset.playerId);
    const action = await this.orm.call(
      "fpl.gameweek.pick.lines",
      "action_open_player_stats",
      [playerId],
      {}
    );
  ...
  getTotalPoints(playerId) {
    return this.state.playersData[playerId]?.total_points || 0;
  }

  getTeamcode(playerId) {
    return this.state.playersData[playerId]?.team_code || "";
  }
